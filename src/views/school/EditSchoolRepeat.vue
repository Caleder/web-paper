<template>
  <div class="el-index">
    <el-form ref="form" :model="form" label-width="110px">
      <el-form-item label="院校ID" prop="id">
        <el-input disabled v-model="form.id" placeholder="请输入院校ID" style="width:290px"/>
      </el-form-item>
    ...
    <div slot="footer" class="dialog-footer">
      ...
      <el-button type="primary" @click="editManage(form)">立即提交</el-button>
    </div>
  </div>
</template>
...
<script>
  export default {
    ...
    methods: {
      ...
      fetchManage(id) {
        this.$axios.get('/schoolRepeat/getSchoolRepeat/' + id).then((result) => {
      ...
      editManage(form) {
        if (!form.schoolName || !form.score
          ...
          this.$axios.post("/schoolRepeat/updateSchoolRepeat", this.form)
            .then((result) => {
              //关闭父组件中的编辑弹框
              this.$layer.close(this.layerid);
              //弹出提示信息，默认时间为3秒
              this.$layer.msg("修改院校复试信息成功！");
              this.reload();
            });
        }
      }
